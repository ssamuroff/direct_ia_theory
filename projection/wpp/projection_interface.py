...
def setup(options):
    ...
    return (sample_a,sample_b,timing,include_lensing,constant_sigmaz)

def execute(block, config):
    sample_a, sample_b, timing, include_lensing, constant_sigmaz = config
    ...
    P_II_interpolator = interp2d(k_power,chi_power,P_II)
    ...
    if include_lensing:
        ...
        P_GI = block['matter_intrinsic_power','p_k']
        ...
        for i, l in enumerate(ell):
            ...
            P1d_GI = [P_GI_interpolator((l+0.5)/x, x) for x in chi]
            #P1d_mG = np.diag(np.fliplr(P_mG_interpolator((l+0.5)/chi, chi)))
            P_GI_2d.append(P1d_GI)
    ...
    for i, z_l in enumerate(z_low):
        for j,pi in enumerate(Pi):
            ...
            if include_lensing:
                C_GG = do_limber_integral(ell, P_GG_2d, gz1, gz2, chi)
                Cell_all[i,j,:]+=C_GG

                C_GI = do_limber_integral(ell, P_GI_2d, gz1, Pz2, chi)
                C_IG = do_limber_integral(ell, P_GI_2d, Pz1, gz2, chi)
                Cell_all[i,j,:]+=C_GI
                Cell_all[i,j,:]+=C_IG
