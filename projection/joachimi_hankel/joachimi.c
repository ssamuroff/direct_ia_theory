...
void * setup(c_datablock * options)
{
	...
	status |= c_datablock_get_string_default(options, OPTION_SECTION, "sample_a", "sample_a", &(config->sample_a));
	status |= c_datablock_get_string_default(options, OPTION_SECTION, "sample_b", "sample_b", &(config->sample_b));
	...
	else if (corr_type==gp1h){
	  status |= c_datablock_get_string_default(options, OPTION_SECTION, "input_section_name", "galaxy_intrinsic_power_1h_projected", &(config->input_section));
	  status |= c_datablock_get_string_default(options, OPTION_SECTION, "output_section_name", "galaxy_intrinsic_w_1h", &(config->output_section));
	...
	if (status){
	  fprintf(stderr, "Please specify input_section_name, output_section_name, and corr_type=0,1,2,3,4 or 5.\n");
	  exit(status);
	}
...
int execute(c_datablock * block, void * config_in)
{
	...
	if (c_datablock_has_value(block, config->input_section, "npi")){
		status |= c_datablock_get_int(block, config->input_section, "npi", &num_pi);
		status |= c_datablock_get_int(block, config->input_section, "nzm", &num_z);
	}
	else{
		status |= c_datablock_get_int(block, config->input_section, "npi", &num_pi);
		num_z = num_pi;
	...
	double * ell;
	int n_ell;
	printf("Converting c_ell to xi(r_p): \n");
	...
	for (int i_bin=0; i_bin<=num_z; i_bin++) {
		for (int j_bin=0; j_bin<=num_pi; j_bin++) {
		    //// Load the C_ell, and compute w
			double * C;
			snprintf(name_in, 64, "c_ell_%s_%s_%d_%d",config->sample_a,config->sample_b,i_bin,j_bin);
			if (!c_datablock_has_value(block, config->input_section, name_in)) continue;
			found_any=true;
			status |= c_datablock_get_double_array_1d(block, config->input_section, name_in, &C, &n_ell);
			if (status){
				fprintf(stderr, "Could not load C(ell)");
				return status;
			}
			...
			switch(config->corr_type) {
				...
				case gp1h:
				    tpstat = tp_w;
				    sprintf(name, "w_rp_1h_%d_%d_%s_%s", i_bin, j_bin, config->sample_a, config->sample_b);
				    snprintf(name_w1, 64, name);
				    coeff = -1.0 ; // * sqrt(2) ;///2.0/PI;
				    break;
				case pp:
			...
			if (neg_vals == 0) {
				cl_table = init_interTable(n_ell, log_ell_min, log_ell_max, dlog_ell, 1.0, -3.0, &err);
				for (int i=0; i<n_ell; i++) cl_table->table[i] = log(C[i]);
			}

		    else if (neg_vals == n_ell){
		        //In this case all the C(ell) elements are negative, so interpolate in log(-C)
...
//
		    	...
		    	static int warned=0;
		    	if (warned==0){
		    		printf("Negative values in C(ell). No interpolation in log(C(ell)),\n");
		    	...
		    	cl_table = init_interTable(n_ell, log_ell_min, log_ell_max, dlog_ell, 0., 0., &err);

		    	for (int i=0; i<n_ell; i++) cl_table->table[i] = C[i];
		    }
		    ...
		    ////Now save to block
		    c_datablock_put_int(block, config->output_section, "nbin_A", num_pi);
		    c_datablock_put_int(block, config->output_section, "nbin_B", num_z);
	...
	c_datablock_put_double_array_1d(block, config->output_section, "theta", rp_vals, N_thetaH);
